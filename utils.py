...
# === Env & Clients ===
load_dotenv()
# API keys are now typically picked up by LangChain models from environment variables
...
def get_response(model: str, prompt: str) -> str:
    if "claude" in model.lower() or "anthropic" in model.lower():
        ...
        llm = ChatGoogleGenerativeAI(model=model, temperature=0, convert_system_message_to_human=True)
    else:  # Default to OpenAI
        llm = ChatOpenAI(model=model, temperature=0, max_tokens=1000)
...
def print_html(content: Any, title: str | None = None, is_image: bool = False, file_path: str | None = None):
    """
    Pretty-print inside a styled card and optionally save to an HTML file.
    ...
    - If file_path is provided, the full HTML content will be saved to this path.
    """
    try:
    ...
    if file_path:
        try:
            ...
            print(f"Error saving content to file {file_path}: {e}")


def image_anthropic_call(model_name: str, prompt: str, media_type: str, b64: str) -> str:
    """
    Call Anthropic Claude with text+image using LangChain.
    """
    llm = ChatAnthropic(model=model_name, temperature=0, max_tokens=2000)
...
def image_openai_call(model_name: str, prompt: str, media_type: str, b64: str) -> str:
    """
    Call OpenAI with text+image using LangChain.
    """
...
def image_gemini_call(model_name: str, prompt: str, media_type: str, b64: str) -> str:
    """
    Call Google Gemini with text+image using LangChain.
    """
